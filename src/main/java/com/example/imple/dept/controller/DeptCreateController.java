...
public class DeptCreateController implements CreateController<DeptDTO> {
	...
	@Override
	@Transactional
	public String create(DeptDTO dto, BindingResult binding, Model model, HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		try {
			...
			binding.reject("duplicatekey", "부서번호가 중복됩니다.");
			
			return "redirect:/dept/create?error";
		}
		
		return "redirect:/dept/success?create"; //post이후 꼭 redirect를 해야한다.
	}
}
